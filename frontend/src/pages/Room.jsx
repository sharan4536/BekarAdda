      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <header className="bekar-header !relative !shadow-none border-b border-indigo-500/20 px-4 sm:px-6 z-20 flex-wrap !py-2 shrink-0">
            ...
            <div className="header-center flex flex-1 items-center justify-center gap-2 sm:gap-3">
                 ...
                 <div className="flex bg-slate-800/80 rounded-xl overflow-hidden shadow-lg border border-white/10">
                     <button 
                         onClick={() => {
                             const text = `Bro 😂 nenu BekarAdda lo match chusthunna… full timepass jarugutundi 🔥\nNuvvu kuda join avvu ra! 👇\n${window.location.href}`;
                             navigator.clipboard.writeText(text);
                             setToastMessage("Link copied ra 🔥");
                             setTimeout(() => setToastMessage(""), 3000);
                         }}
                         className="hover:bg-indigo-600 px-3 py-2 transition-colors flex items-center border-r border-white/10"
        ...
        {/* Global Action Toast Injection */}
        {toastMessage && (
            <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1000] animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="bg-emerald-500/90 text-white font-bold px-6 py-3 rounded-full shadow-[0_10px_40px_rgba(16,185,129,0.4)] border border-white/20 backdrop-blur-md flex items-center gap-2">
                    ✅ {toastMessage}
                </div>
            </div>
        )}
